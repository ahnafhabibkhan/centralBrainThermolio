...
def build_mcp(settings, repo):
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "In connected chats, end each final assistant response with one brief Central Brain status line. "
            "If the conversation introduces new durable information, ask: Central Brain: Would you like to save [specific summary]? "
            "If there is nothing new worth saving, say: Central Brain: There is nothing new to save. "
            "After a successful proposal, report that it is awaiting approval instead of asking to save it again. "
            ...
            "For GitHub imports, fetch a fresh download_url for each file at the inventoried commit immediately before importing it. "
            "Do not queue temporary URLs for the whole project. On an authorization or missing-file response, refresh only that file URL and retry once. "
            "Recheck transfer status before declaring a batch incomplete. Central Brain cannot renew a private GitHub URL without source authorization. "
            ...
            "for the original. Use that downloaded file for the original upload and preserve its filename and bytes. "
            "Do not silently replace an original with a text extract. Ask for explicit agreement before saving an "
            "extract instead, and report the original as still missing. A partial export is not a complete export. "
